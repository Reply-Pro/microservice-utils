...
def calculate_batch_size(items: List[dict], max_batch_size_mb: int = 2) -> int:
    ...
    Args:
        items: List of items to be upserted, each containing 'values' and optionally 'metadata'
        max_batch_size_mb: Maximum batch size in megabytes (default: 2)
    ...
    for item in items[:sample_size]:
        ...
        if 'metadata' in item:
            ...
            if metadata_size > max_metadata_size:
                raise ValueError(f"Metadata size ({metadata_size} bytes) exceeds maximum allowed (40KB)")

        # Add some overhead for JSON structure and id
        item_size = vector_size + metadata_size + len(item.get('id', '')) + 100  # 100 bytes overhead
